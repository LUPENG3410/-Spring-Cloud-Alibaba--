...
|------|------|------|------|
| brand | string | 否 | 品牌筛选 |
| type | string | 否 | 类型筛选: sedan/suv/mpv/luxury/sports |
...
```
DELETE /admin/trims/:id
```

**说明**: 仅当该车款下无库存车辆时才允许删除。
...
**说明**: 获取所有库存车辆，包含车款和门店信息。

**查询参数**:
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| storeId | number | 否 | 门店筛选 |
| trimId | number | 否 | 车款筛选 |
| status | string | 否 | 状态筛选: available/rented/maintenance |
...
| features | string[] | 配置标签列表 |
| descriptions | string[] | 车辆描述列表 |
| status | number | 状态: 1-在售 0-停售 |
| stockCount | number | 库存数量(统计字段) |
...
| 车款 | `GET /cars`, `GET /cars/:id` | `GET /admin/trims`, `POST /admin/trims`, `PUT /admin/trims/:id/status`, `DELETE /admin/trims/:id` |
| 库存车辆 | - | `GET /admin/cars`, `POST /admin/cars`, `PUT /admin/cars/:id/status`, `DELETE /admin/cars/:id` |
